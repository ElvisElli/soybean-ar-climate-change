...
p4_data <- p3_data %>%
  mutate(scenario = factor(scenario, labels = scenario_labels)) %>%
  group_by(x, y, scenario, co2) %>%
...
## ── Figure 5: Adaptation strategies — merged (Fig 3 + Fig 4) ─────────── ----
## Combined manuscript figure

plot5_merged <- plot_grid(
  plot3 + theme(plot.margin = margin(5, 5, 5, 5)),
  plot4  + theme(plot.margin = margin(5, 5, 5, 5)),
...
p6b_pheno <- simulated0 %>%
  filter(scenario %in% c("baseline", "climate_change"), co2 == 350) %>%
  mutate(year = year(date)) %>%
  select(x, y, scenario, year, EmergenceDAS, FloweringDAS, MaturityDAS) %>%
  left_join(
    p6_weather %>% select(x, y, year, weather.class, Meat.avg, Rain.avg, Meat.sd, Rain.sd),
    by = c("x", "y", "year")) %>%
