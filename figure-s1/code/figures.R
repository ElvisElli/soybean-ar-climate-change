...
p3 <- ggplot()+
  geom_point(data = uncal.phen, aes(x = dap.sim, y = dap.obs, fill = 'Uncalibrated', , pch = 'Uncalibrated', colour = 'Uncalibrated'))+
  geom_point(data = cal.phen, aes(x = dap.sim, y = dap.obs, fill = 'Optimized', pch = 'Optimized', colour = 'Optimized'))+
  geom_text(aes(x = Inf, y = -Inf, label = cal.label),
            vjust = -0.1, hjust = 1.1)+
  geom_text(aes(x = -Inf, y = Inf, label = pre.label),
            vjust = 1, hjust = 0)+
