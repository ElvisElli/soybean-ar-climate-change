...
##p4b/p4c - phenology change maps (without elevated CO2 only) ====
...
## helper: build stars + warp
to_stars <- function(df) {
...
plot4b <-
  ...
  facet_wrap(~ scenario, nrow = 1) +
  temp +
  scale_fill_manual(values = pheno_colors, na.value = "grey50", na.translate = FALSE,
                    name = "B  Seed-filling period change (days)") +
  map_theme
...
plot4c <-
  ...
  facet_wrap(~ scenario, nrow = 1) +
  temp +
  scale_fill_manual(values = cycle_colors, na.value = "grey50", na.translate = FALSE,
                    name = "A  Total crop cycle change (days)") +
  map_theme

## merged panel: A = total cycle (upper), B = seed-fill (lower)
plot_pheno_merged <- plot_grid(
  plot4c, plot4b,
  ncol        = 1,
  align       = "v",
  axis        = "lr"
...
ggsave("figures/p4bc - phenology change maps.tiff", plot = plot_pheno_merged,
