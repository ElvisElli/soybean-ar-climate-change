## ============================================================
## us-irrigated-corn-map.R
## US county-level maps of corn irrigated area from USDA Census
## of Agriculture (most recent = 2022).
##
## Figures:
##   fig_corn_irrigated_area.tiff      — irrigated corn area (acres) per county
##   fig_corn_irrigated_pct.tiff       — irrigated as % of total corn area
## ============================================================
...
## ── Step 1: Download county-level corn data from NASS Census ─────────────────
fetch_nass <- function(commodity, short_desc) {
...
raw_total    <- fetch_nass("CORN", "CORN, GRAIN - ACRES HARVESTED")
raw_irrigated <- fetch_nass("CORN", "CORN, GRAIN, IRRIGATED - ACRES HARVESTED")
...
corn_df <- left_join(total_df, irrigated_df, by = "fips") %>%
  mutate(
...
n_known    <- sum(!is.na(corn_df$acres_irrigated), na.rm = TRUE)
n_withheld <- sum(isTRUE(corn_df$irrig_withheld) | corn_df$irrig_withheld, na.rm = TRUE)
message(sprintf("[Map] %d counties with total corn | %d with known irrigated area | %d withheld (D)",
...
state_stats <- corn_df %>%
  ...
  summarise(
    ...
    total_corn_acres    = sum(acres_total,    na.rm = TRUE),
    irrig_acres         = sum(acres_irrigated, na.rm = TRUE),
    pct_irrigated       = round(100 * irrig_acres / total_corn_acres, 1),
...
cat(sprintf("\n── State-level irrigated corn summary (%d Census) ──\n", CENSUS_YEAR))
print(state_stats, n = Inf)

write.csv(state_stats, "figures/state_irrigated_corn_stats.csv", row.names = FALSE)
message("[Map] Saved: figures/state_irrigated_corn_stats.csv")
...
## ── Figure 1: irrigated corn area (acres) ────────────────────────────────────
message("[Map] Building fig_corn_irrigated_area...")
...
plot_area <- ggplot() +
  ...
  labs(
    title    = "Irrigated Corn Area — US Counties",
    subtitle = paste0("USDA Census of Agriculture ", CENSUS_YEAR,
                      "  |  grey = no irrigation / <500 acres  |  tan = data withheld  |  sqrt colour scale")
  ) +
  map_theme

ggsave("figures/fig_corn_irrigated_area.tiff", plot = plot_area,
       width = 25, height = 15, units = "cm",
       dpi = 600, compression = "lzw", bg = "white")
message("[Map] Saved: figures/fig_corn_irrigated_area.tiff")

## ── Figure 2: irrigated as % of total corn area ──────────────────────────────
message("[Map] Building fig_corn_irrigated_pct...")
...
plot_pct <- ggplot() +
  ...
  annotate("text", x = Inf, y = -Inf, hjust = 1.05, vjust = -0.5, size = 2.8,
           ...
           label = paste0("■ = irrigation present, amount withheld by NASS (n = ", n_withheld, " counties)")) +
  labs(
    title    = "Irrigated Corn as % of Total Corn Area — US Counties",
    subtitle = paste0("USDA Census of Agriculture ", CENSUS_YEAR,
                      "  |  grey = no irrigation / <500 acres  |  tan = data withheld  |  % of county corn area")
  ) +
  map_theme

ggsave("figures/fig_corn_irrigated_pct.tiff", plot = plot_pct,
       width = 25, height = 15, units = "cm",
       dpi = 600, compression = "lzw", bg = "white")
message("[Map] Saved: figures/fig_corn_irrigated_pct.tiff")

## ── Console summary ───────────────────────────────────────────────────────────
cat(sprintf("\n── US Irrigated Corn Summary (%d Census) ──\n", CENSUS_YEAR))
corn_df %>%
  summarise(
    total_corn_Macres     = round(sum(acres_total,    na.rm = TRUE) / 1e6, 2),
    irrigated_corn_Macres = round(sum(acres_irrigated, na.rm = TRUE) / 1e6, 2),
